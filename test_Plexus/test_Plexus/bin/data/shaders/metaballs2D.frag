...
void doCamera(out vec3 camPos, out vec3 camTar, in float time, in vec2 mouse)
{
	...
	float theta = 0.3 + 5.0*mouse.x - u_Time*0.5;
	float phi = 3.14159*0.4;
	//float phi = 5.0*mouse.y;
...
vec2 map(vec3 pos) {
	...
	float sphere = distSphere(pos, u_Radius) + u_Noise * noise(pos * 1.0 + u_Time*0.75);
	float t1 = sphere;


	//for(int i = 0; i < 2; i++) {
		//float rad = map(u_Balls[i].y, 0.0, u_Radius*2, 0.01, 0.0);
		t1 = smin(t1, distSphere(pos + vec3(-1.8, 0.0, 0.0)/*u_Balls[i].xyz*/, 0.1), 2.0);
		t1 = smin(t1, distSphere(pos + vec3(1.8, 0.0, 0.0)/*u_Balls[i].xyz*/, 0.1), 2.0);
...
vec2 map2(vec3 pos) {
	...
	//float sphere = distSphere(pos, 1.0) + noise(pos * 1.2 + vec3(-0.3) + u_Time*0.2);
...
void renderColor2(vec3 ro, vec3 rd, inout vec3 color, vec3 currPos)
{
	...
	vec3 normal_distorted = calcNormal2(currPos + rd*noise(currPos*2.5 + u_Time*2.0)*0.75);
...
void renderColor(vec3 ro, vec3 rd, inout vec3 color, vec3 currPos)
{
	...
	vec3 normal_distorted = calcNormal(currPos + noise(currPos*1.5 + vec3(0.0, 0.0, sin(u_Time*0.75))));
